...
const HeroSection = () => {
  return (
    <section
      className="relative mx-auto w-full max-w-screen-xl overflow-hidden rounded-[28px] bg-black text-white px-4 sm:px-6 md:px-8 min-h-[100svh]"
      aria-label="Hero"
    >
      {/* Top visual area */}
      <header className="relative">
        ...
        <div
          className="absolute inset-x-0 top-[56%] sm:top-[54%] md:top-[52%] lg:top-[50%]"
          style={{
            // Single source of truth for angle in both top card and stats
            // Tune this percentage to steepen/relax the diagonal consistently
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] gap-6 sm:gap-8 lg:gap-10">
            ...
            <div className="relative order-2 md:order-1">
              <div
                className="text-left rounded-[28px] bg-[#161616]/95 p-5 sm:p-6 md:p-8 lg:p-10 shadow-xl"
                style={{
                  clipPath: "polygon(0 var(--hero-cut), 86% calc(var(--hero-cut) + 22%), 86% 100%, 0% 100%)",
                }}
              >
                ...
                <div className="mt-6 sm:mt-7 md:mt-8 flex flex-wrap items-center gap-3 sm:gap-4">
                  <button
                    type="button"
      ...
      <div className="mt-4 ml-72 sm:mt-6 md:mt-40 relative z-20 flex flex-wrap items-stretch justify-center md:justify-start gap-4 sm:gap-5 pb-8 sm:pb-10">
        ...
        {[0, 1].map((i) => (
          ...
          >
            <h3 className="leading-none font-extrabold text-accent text-[clamp(1.75rem,4.2vw,3rem)] tracking-tight">100</h3>
            <p className="mt-2 text-[11px] sm:text-xs md:text-sm text-gray-300">Lorem ipsum dolor sit</p>
          </div>
        ))}
      </div>
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 rounded-[28px] bg-gradient-to-br from-black/70 via-transparent to-accent/5"
      />
    </section>
  );
};
